...
function linkIcon(type = "") {
  if (type === "facebook") {
    return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13.5 22v-8h2.7l.4-3.1h-3.1V8.9c0-.9.3-1.5 1.6-1.5H16.7V4.6c-.3 0-1.3-.1-2.4-.1-2.4 0-4 1.4-4 4.2v2.2H7.7V14h2.6v8h3.2Z"/></svg>';
  }
...
function renderLinks(links = []) {
  ...
  return `
    <div class="client-profile-links" aria-label="External links">
      ${links.map((link) => `
        <a class="client-profile-link" href="${escapeClient(link.url)}" target="_blank" rel="noreferrer" aria-label="${escapeClient(link.label || link.type || 'Visit link')} for ${escapeClient(link.clientName || '')}" title="${escapeClient(link.label || link.type || 'Visit link')}">
          ${linkIcon(link.type)}
        </a>`).join("")}
    </div>`;
}
...
function clientCard(item) {
  const links = Array.isArray(item.links)
    ? item.links.map((link) => ({ ...link, clientName: item.name }))
    : [];
